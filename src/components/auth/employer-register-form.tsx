...
import { employerRegisterAction } from "@/lib/actions/auth";
import { TextField } from "@/components/ui/field";
import { Button } from "@/components/ui/button";

export function EmployerRegisterForm() {
  const [state, formAction, isPending] = useActionState(employerRegisterAction, undefined);
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4" noValidate>
      ...
      <TextField label="Work email" name="email" type="email" autoComplete="email" required error={state?.fieldErrors?.email} />
      <TextField label="Company name" name="companyName" required error={state?.fieldErrors?.companyName} />
      <TextField
        label="Password"
        name="password"
        type="password"
      ...
      <label className="flex items-start gap-2.5 text-sm text-[var(--color-text-secondary)]">
        ...
        <span>Send me occasional emails about product updates (optional).</span>
      </label>
